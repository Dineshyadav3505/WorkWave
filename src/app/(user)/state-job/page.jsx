...
const Page = () => {
  ...
  const handleState = (state) => {
    setCallState(state); // Update the state for future requests
    setFilter(false); // Close the filter
    setCurrentPage(1); // Reset to first page
    fetchProducts(1, state); // Pass the selected state directly to fetchProducts
  };

  const fetchProducts = useCallback(
    async (page, selectedState = callState) => {
      setLoading(true);
      try {
        const response = await axios.get("/api/post", {
          params: {
            ...
            link: "applyLink",
            state: "true",
            stateName: selectedState, // Use the selected state directly
          },
        });
  ...
  }, [searchResult, currentPage, callState]); // Run effect when search result, current page, or state changes
